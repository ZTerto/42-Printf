...
/*   Updated: 2024/11/07 12:52:02 by ajodar-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_hex(int num, char *format)
{
	char			toprint[8];
	unsigned int	unum;
	int				count;
...
static int	ft_format(const char *format, va_list arguments)
{
	...
	if (*format == 'x')
		cnt = ft_hex(va_arg(arguments, int), "0123456789abcdef");
	if (*format == 'X')
		cnt = ft_hex(va_arg(arguments, int), "0123456789ABCDEF");
	if (*format == 'p')
